...
- **`warm` — compile+exec vs. exec-only.** The Node `warm` loop re-runs `api.run(script, inputs)` each iteration, which includes compilation (the wrapper caches compiled scripts internally, so steady-state is exec-dominated). To keep the delta apples-to-apples, the engine `warm` should match the wrapper's effective steady-state behavior. Default: re-run `compile+write` per iteration and let the JIT + any engine-side compile caching settle during the 2000-iter warmup, so `warm` reflects the same steady state the wrapper reports. If profiling shows engine compilation is not cached across iterations (unlike the wrapper), switch `warm` to time an already-compiled `DataWeaveScript.write` and document the asymmetry. Decide during implementation by measuring; record the choice in `WarmBench.scala`.
